...
- 将详情整理为“活动与读数”“历史与任务”“电池与散热”“设置与节能”四页，增加外观选择与菜单快捷键。
- 增加两次任务的能耗、耗时、有效覆盖率和平均功率比较。
- 增加最近 90 天的电池健康摘要和最多 180 条睡眠记录；睡眠耗电明确标注为端点估算，原始电量／容量／循环数缺失时保留未知，不回填为 0。
- 增加风扇当前、目标、上下限转速与模式的只读显示，保留真实 0 RPM；充电上限使用系统设置入口。
- 将历史及电池档案的紧凑 JSON 编码、原子保存移到串行后台队列，退出时等待最新快照写完；保留损坏档案副本。
- 增强服务升级为 XPC v2，支持 2／5／10 秒白名单采样档位，以及用户主动操作的电池／接电低功耗模式切换与回读确认。自动启动和重连不发起安装授权。
- 补充通知偏好、后台保存、原始电池字段、风扇和 helper 策略回归，以及隔离数据目录、明暗界面渲染和 Debug 生命周期集成检查入口。

低功耗设置已覆盖模拟执行、参数限制和回读失败等测试；本轮未实际执行 root 写设置验证。不同机型、真实睡眠唤醒和管理员写入仍需单独验收。

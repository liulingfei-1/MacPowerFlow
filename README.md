...
- 用连续块状能量流展示适配器、电池、整机、CPU、GPU、显示和“其他”残差；各流道宽度与实时功耗联动，文字会跟随流道厚度自动换行或压缩。
...
- “其他”明确标成残差估算，并细分解释 ANE、内存、媒体引擎、SSD、网络、雷雳/USB 控制器、风扇、电源转换和采样时差等可能来源；不会虚构没有可靠传感器的独立瓦数。
- 列出当前 CPU 占用排序靠前的最多三个进程；这里的百分比不是逐应用功耗瓦数。
- 对缺失或暂时不可用的传感器显示“—”；拿不到管理员 CPU 字段时，会根据可见 SoC 残差或整机负载与 CPU 活跃度给出保守估算，并用“≈”明确标识。
...
3. 生成并解包复验不携带 Finder 扩展属性的 `dist/MacPowerFlow-1.4.0.zip`；
...
启动后，MacPowerFlow 会出现在菜单栏并定期刷新采样。第一次使用这个版本时，macOS 会显示一次管理员认证；通过后，应用安装固定用途的增强服务并立即开始采样。以后重新打开应用或重启电脑都直接连接该服务，不再要求密码。左键点击图标可打开功耗面板；右键点击可立即刷新、设置登录时启动、查看“关于”或退出应用。用于截图和界面验收的 `--preview` 启动参数会跳过增强服务安装与连接。
...
管理员流提供 Apple 单独估算的 CPU、GPU、ANE、CPU+GPU+ANE 合计、频率、活跃度和热压力。主视觉中的 CPU / GPU 会逐项优先使用最近且仍然新鲜的增强数据；CPU 字段缺失或授权未完成时，CPU 会回退到明确标有“≈”的本地估算。GPU 缺失时仍使用普通 IOReport 可见值，不会虚构数据。

`powermetrics` 自己也明确说明这些功耗数字是模型估算值，可能不准确，而且不适合跨设备比较。因此界面仍用 `≈` 标识 CPU / GPU，`CPU+GPU+ANE` 合计也不会被冒充成整机功耗。“其他”仍由整机负载扣除 CPU、GPU 与显示得到，避免把两种不同口径混为一谈。

当前 ZIP 使用 hardened ad-hoc 签名，安装时会精确绑定当前 1.4.0 App。因此这个版本安装一次后，日常启动与系统重启都不再输入密码；若以后替换 App 二进制，CDHash 会变化，升级后的版本必须再批准一次。这样避免让旧 root helper 无条件信任任意新文件。

若要正式分发并在升级后继续保持稳定身份，应使用同一 Team 的 Developer ID Application 签名、公证，并迁移到 Apple 推荐的 `SMAppService` LaunchDaemon。当前机器没有 Developer ID 身份，因此 1.4.0 采用本机可工作的固定版本安装方式，而没有伪装成一个实际上无法获批的 `SMAppService` 包。
...
| 整机、SoC 热功耗、显示功耗和芯片温度 | AppleSMC 只读键，例如 `PSTR`、`PHPC`、`PDBR` | 私有、未文档化接口；`PHPC` 在界面中明确标为推定值 |
| CPU/GPU/ANE/DRAM、GPU SRAM、媒体、ISP、Fabric、PCIe、显示控制器功耗 | IOReport 的 Energy Model / Energy Counters 通道 | 私有、未文档化接口；功率由相邻能量样本的差值计算，不同机型只显示实际存在的通道 |
...
| CPU 回退估算 | 可见 SoC 功耗残差；缺失时使用整机负载与 CPU tick 活跃度的保守曲线 | 仅在拿不到新鲜管理员 CPU 字段和普通 CPU 能量通道时使用，始终显示 `≈`，不是硬件传感器实测 |
